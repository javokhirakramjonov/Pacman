...
public class Repository {
    ...
    private volatile boolean canDeath = true;
    ...
    private void createPacman() {
        ...
        board[pacman.getPosition().x][pacman.getPosition().y].remove(CellModel.FOOD);
        setLifeRefresh();
    }

    private void setLifeRefresh() {
        if (deathTimer != null) {
            deathTimer.interrupt();
        }
        deathTimer = new Thread(() -> {
            try {
                canDeath = false;
                Thread.sleep(2000);
                canDeath = true;
    ...
    private void createGhosts() {
        IntStream.range(0, ghostsCount).forEach(i -> ghosts.add(new GhostModel(CELL_SIZE)));
    ...
    private void addFoods() {
        Arrays.stream(board).flatMap(Arrays::stream).filter(List::isEmpty).forEach(cell -> {
            cell.add(CellModel.FOOD);
            foodCount++;
        });
    }
    ...
    private void addBorders() {
        ...
        if (horizontalDoor) {
            ...
            IntStream.range(0, rows).filter(i -> !board[i][1].contains(CellModel.BLOCK)).forEach(freeLeft::add);
            List<Integer> freeRight = new ArrayList<>();
            IntStream.range(0, rows).filter(i -> !board[i][columns - 2].contains(CellModel.BLOCK)).forEach(freeRight::add);
            ...
            IntStream.range(0, columns).filter(i -> !board[1][i].contains(CellModel.BLOCK)).forEach(freeTop::add);
            List<Integer> freeBottom = new ArrayList<>();
            IntStream.range(0, columns).filter(i -> !board[rows - 2][i].contains(CellModel.BLOCK)).forEach(freeBottom::add);
        ...
        IntStream.range(1, rows - 1).filter(row -> !board[row][0].contains(CellModel.HORIZONTAL_DOOR) && !board[row][0].contains(CellModel.VERTICAL_DOOR)).forEach(row -> board[row][0].add(CellModel.LEFT_BORDER));

        IntStream.range(1, rows - 1).filter(row -> !board[row][columns - 1].contains(CellModel.HORIZONTAL_DOOR) && !board[row][columns - 1].contains(CellModel.VERTICAL_DOOR)).forEach(row -> board[row][columns - 1].add(CellModel.RIGHT_BORDER));

        IntStream.range(1, columns - 1).filter(column -> !board[0][column].contains(CellModel.HORIZONTAL_DOOR) && !board[0][column].contains(CellModel.VERTICAL_DOOR)).forEach(column -> board[0][column].add(CellModel.TOP_BORDER));

        IntStream.range(1, columns - 1).filter(column -> !board[rows - 1][column].contains(CellModel.HORIZONTAL_DOOR) && !board[rows - 1][column].contains(CellModel.VERTICAL_DOOR)).forEach(column -> board[rows - 1][column].add(CellModel.BOTTOM_BORDER));
    }
    ...
    private void actGhosts() {
        ...
        ghostThread = new Thread(() -> {
            while (true) {
                try {
                    Thread.sleep(ghosts.get(0).getSpeed());
    ...
    private void moveFreeDirection(GhostModel ghost) {
        ...
        for (Direction direction : Direction.values()) {
            if (canMove(x, y, x + directionX[nextPos.get(direction)], y + directionY[nextPos.get(direction)], direction)) {
    ...
    public List<Object> getCellElements(int rowIndex, int columnIndex) {
        ...
        List<CellModel> cell = getBoard()[rowIndex][columnIndex];
        if (pacman.getPosition() != null && pacman.getPosition().equals(position)) {
            ...
            elements.add(pacman);
            pacman.setEating(cell.contains(CellModel.FOOD));
            if (cell.contains(CellModel.FOOD)) {
                currentScore++;
                if (currentScore == foodCount) {
                    stop();
                }
                gameStateListener.setScore(currentScore);
                cell.remove(CellModel.FOOD);
            }
        }
        elements.add(cell);
        return elements;
    ...
    public void stop() {
        ...
        gameStateListener.finishGame(currentScore, time);
    }
}
